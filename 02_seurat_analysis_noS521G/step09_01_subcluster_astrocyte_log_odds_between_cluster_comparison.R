...
outdir <- paste0(basedir, 
                 "09_subclusters_log_odds_calculation/astrocyte_without_gender_effect/")
...
estimateCellStateChange <- function(k, counts, pheno) {
  ...
  pdf(paste0("proportion_of_cells_per_genotype_groupby_sex_astrocyte_subcluster_",
             k,
             ".pdf"))
  print(ggplot(cluster_counts, 
               aes(x=animal_model, 
...
write.csv(ClusterRes, 
          file = "log_odds_ratio_without_gender_effect_per_genotype_per_astrocyte_subcluster.csv")
...
label.df$logOddsRatio <- 2.9
label.df2 <- ClusterRes_plot[ClusterRes_plot$pvalue < 0.05,]
label.df2$logOddsRatio <- 2.2
pdf("log_odds_boxplot_without_gender_effect_astrocyte_subcluster_with_padj.pdf",
    height = 20,
    width = 35
)
...
pdf("log_odds_boxplot_without_gender_effect_astrocyte_subcluster_with_pvalue_and_padj.pdf",
    height = 20,
    width = 35
)
ggplot(ClusterRes_plot, 
