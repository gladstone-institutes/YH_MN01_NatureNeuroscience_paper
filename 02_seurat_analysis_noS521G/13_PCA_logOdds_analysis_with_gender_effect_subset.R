...
logOdds_histo_micro=read.csv(paste0(logOdds_micro_dir,"microglia_with_gender_effect_log_odds_ratio_per_unit_per_histopathology_per_cluster_by_sample.csv"))
...
p <- fviz_pca_ind(res.pca.norm.histo,
             geom.ind = c("point"),
                ...
                #geom_text(aes(label=ifelse(data.frame(res.pca.norm.histo$x)$PC1>2 & data.frame(res.pca.norm.histo$x)$PC2>0,as.character(pca_histo_log$cluster),'')),position = position_dodge(0.9))
                 annotate("text", label = "MG cluster 11", x = -3.85 ,y = -1.31, size = 4) +
                annotate("text", label = "MG cluster 8", x = 2.3 ,y = -0.525, size = 4) +
                annotate("text", label = "MG cluster 2", x = -1.6 ,y = 0.72, size = 4) +
                annotate("text", label = "AS cluster 3",x = 0.9 ,y = 0.165, size = 4) +
                annotate("text", label = "AS cluster 5", x = 2.2 ,y = -0.06, size = 4) +
                annotate("text", label = "AS cluster 7", x = 2.9 ,y = -0.17, size = 4) +
                 annotate("text", label = "Cluster 1", x = 0.31 ,y = 0.31, size = 4) +
                 annotate("text", label = "Cluster 6", x = -1.8 ,y = 0.44, size = 4) +
                 annotate("text", label = "Cluster 7", x = -0.2,y = 0.37, size = 4)+
                 annotate("text", label = "Cluster 9", x = 2.45 ,y = -0.35, size = 4)+
                 annotate("text", label = "Cluster 28", x = -1.54 ,y = 0.38, size = 4)
  ggsave(file = file.path(outdir, paste0("PCA_logodds_histopathology_cl1_6_7_9_28_astro_3_5_7_micro_2_8_11_with_gender_effect.pdf")),
         plot = p,
         width = 10,
         height = 10)
