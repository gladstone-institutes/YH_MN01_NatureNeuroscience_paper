...
dat <- readRDS(paste0(indir,
                      "microglia_data_post_subcluster_no_S521G_pcadims_15_res_0.9.rds"))
...
write.csv(smp_cluster_counts, 
          file = paste0(outdir,"microglia_counts_per_sample_per_cluster.csv"),
          row.names = FALSE)
...
estimateCellStateChange <- function(k, counts, histopheno, optimizer) {
  ...
  cluster_counts <- smp_cluster_counts %>% 
    filter(cluster_id == k)
...
pdf(paste0(outdir,"microglia_proportion_of_cells_per_sample_cluster_",k,"_per_pheno_",colnames(cluster_counts[ph]),".pdf"))
...
ClusterRes <- sapply(Clusters, estimateCellStateChange, smp_cluster_counts, histopheno)
...
print(ClusterRes)
write.csv(t(ClusterRes), file = paste0(outdir,"microglia_with_gender_effect_log_odds_ratio_per_unit_per_histopathology_per_cluster_by_sample.csv"))
...
histo_all<-read.csv(paste0(outdir,"microglia_with_gender_effect_log_odds_ratio_per_unit_per_histopathology_per_cluster_by_sample.csv"))
...
#subset with cluster of interest 2,8,11.
histo_no_norm_cluster_of_interest <-  histo_no_norm %>% select(Cluster2,Cluster8,Cluster11)
histo_rowMeans_norm_cluster_of_interest <-  histo_rowMeans_norm %>% select(Cluster2,Cluster8,Cluster11)
...
pdf(paste0(outdir,"Fig.ex8h_microglia_with_geneder_effect_pathology_cl2_8_11.pdf"), height=6, width=12)
